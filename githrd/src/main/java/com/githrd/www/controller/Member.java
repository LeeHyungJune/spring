...
//		System.out.println("************** mVO.pw : " + mVO.getPw());
		...
		if(cnt == 1) {
			...
			//	로그 처리
			memberLog.info(mVO.getId() + " 님이 로그인 했습니다.");	//	5.	로그 메세지 출력
...
//		System.out.println("### 관리자");
	...
	*/
	...
	public ModelAndView logout(ModelAndView mv, HttpSession session, String vw, String nowPage) {
		...
		memberLog.info(sid + " 님이 로그인 했습니다.");
...
//			rv.setUrl(vw + "?nowPage=" + nowPage);
		} else {
//			rv.setUrl("/www/member/join.blp?vw=" + vw + "&nowPage=" + nowPage);
